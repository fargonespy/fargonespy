...
public class AvailabilityService implements Runnable {
    ...
    private void handlePacket(DatagramSocket socket, DatagramPacket incomingPacket) throws IOException {
        var data = incomingPacket.getData();
        var peer = incomingPacket.getSocketAddress().toString();
        LOG.info("[{}] Packet type {}", peer, data[0]);
        LOG.info("[{}] Packet dump: {} ", peer, prettyHex.formatHex(data, 0, incomingPacket.getLength()));
        if (data[0] == AVAILABILITY_CHECK) {
            ...
            LOG.info("[{}] Challenge received from client {} vs our challenge {}", peer, prettyHex.formatHex(clientChallenge), prettyHex.formatHex(ourChallenge));
...
//            attrs.put("hostname", "9.9.9.9");
            ...
            if(session == null || session.isChallengePending()) {
                ...
                socket.send(new DatagramPacket(pb, pb.length, incomingPacket.getAddress(), incomingPacket.getPort()));
                return;
            }
