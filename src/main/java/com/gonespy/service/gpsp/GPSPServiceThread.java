...
public class GPSPServiceThread extends Thread {
  ...
  //      uniquesearch - search suggest unique (for suggesting a unique nickname, given a preferred
  // nickname)
    ...
    try (InputStream reader = socket.getInputStream();
      ...
      while (true) {
        ...
        if (directive == null) {
          ...
          // this is necessary in order to populate the friends list when you want to send game
          // invites
          Map<String, String> inputMap = GPMessageUtils.parseClientLogin(clientString);

          // \searchunique\\sesskey\5555\profileid\7777\
          // uniquenick\CSlucher818\namespaces\28\gamename\bstormps3\final\
          ...
          // GPMessageUtils.readGPMessage(reader);
          //                        if(clientRequestString != null && clientRequestString.length() >
          // 0) {
