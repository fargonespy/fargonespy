...
public class GsLargeInt {
  ...
  public static GsLargeInt gsLargeIntSetFromHexString(String hex) {
    ...
    ret.length =
        ((len + (2 * GS_LARGEINT_DIGIT_SIZE_BYTES - 1)) / (2 * GS_LARGEINT_DIGIT_SIZE_BYTES));
    ...
    while (len > 0) {
      if (len >= 2) {
        ...
        temp = ((Character.digit(s.charAt(0), 16) << 4) + Character.digit(s.charAt(1), 16));
      } else {
  ...
  public int gsLargeIntGetByteLength() {
    ...
    */
    ...
    // skip leading zeroes
    while (intSize > 0 && this.data[intSize - 1] == 0) intSize--;
    if (intSize == 0) return 0; // no data
  ...
  // gsi_bool gsLargeIntPowerMod(const gsLargeInt_t *b, const gsLargeInt_t *p, const gsLargeInt_t
  // *m, gsLargeInt_t *dest)
  public static GsLargeInt encrypt(GsLargeInt base, GsLargeInt privateExp, GsLargeInt privateMod) {
    ...
    try {
      ...
      RSAPublicKeySpec pubKeySpec =
          new RSAPublicKeySpec(new BigInteger(modString, 16), new BigInteger(expString, 16));
