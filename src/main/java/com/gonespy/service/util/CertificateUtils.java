...
public abstract class CertificateUtils {
  ...
  public static final String WS_AUTHSERVICE_SIGNATURE_PRIVATE_EXP =
  ...
  // gsi_is_false(gsXmlReadChildAsTStringNT (reader, "profilenick", cert->mProfileNick,
  // WS_LOGIN_NICK_LEN))       ||

  // peerkeymodulus / peerkeyexponent - gsLargeIntSetFromHexString implemented in gonespy. hex ->
  // int array weirdness
  //    GS_CRYPT_RSA_BYTE_SIZE = 128  ,  times 2 chars per byte = 256 characters,  plus one for null
  // termination
  // 		gsi_is_false(gsXmlReadChildAsStringNT (reader, "peerkeymodulus", hexstr,
  // GS_CRYPT_RSA_BYTE_SIZE*2 +1)) ||
  //		gsi_is_false(gsLargeIntSetFromHexString(&cert->mPeerPublicKey.modulus, hexstr)) ||

  // serverdata - WS_LOGIN_SERVERDATA_LEN = 128
  // 		gsi_is_false(gsXmlReadChildAsHexBinary(reader, "serverdata", cert->mServerData,
  // WS_LOGIN_SERVERDATA_LEN, &hexlen)) ||

  // signature - WS_LOGIN_SIGNATURE_LEN = 128
  //		gsi_is_false(gsXmlReadChildAsHexBinary(reader, "signature", cert->mSignature,
  // WS_LOGIN_SIGNATURE_LEN, &hexlen))
  ...
  private static String generateSignature(String md5) {
    ...
    int paddingCount =
        (256
                    ...
                    - CRYPTO_SEPARATOR_BYTE.length()
                    - MD5_HEADER_STRING.length()
                    - md5.length())
                / 2
            - 22;
    ...
    String rawSignature =
        CRYPTO_PREFIX
            + Strings.repeat("FF", paddingCount)
            + CRYPTO_SEPARATOR_BYTE
            + MD5_HEADER_STRING
            + md5;
  ...
  public static Map<String, String> getCertificateEaEmu(Map<String, Object> inputData) {
    ...
    certificateData.put(
        "partnercode", (String) inputData.get("partnercode")); // what is this for c&c ra3?
    certificateData.put(
        "namespaceid", (String) inputData.get("namespaceid")); // what is this for c&c ra3?
